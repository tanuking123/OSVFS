...
| 構成要素 | 用途 |
| --- | --- |
| サブコマンド (`mount` / `mount-all` / `credentials` / `doctor`) | どのマウントを起動するか、暗号化された認証情報ストアの管理、または環境セルフチェック |
...
AWS IAM Identity Center (旧 AWS SSO) を利用する環境では、`osvfs credentials
sso` で OAuth デバイス認可フローを最後まで実行できます。OIDC クライアント
登録 → 既定ブラウザでユーザーポータルを開く → ユーザーが承認するまで
`CreateToken` をポーリング → 取得したベアラトークンと引き換えに短期間ロール
クレデンシャルを取得 → 指定したプロファイル名で保存、まで自動化されます。
...
ベアラトークンと OIDC クライアント登録は `OSVFS:sso-cache:<startUrl>` という
target name で DPAPI 暗号化したうえでキャッシュされます。トークンの有効
期限内であればブラウザを開かずにロールクレデンシャルだけ更新します。
プロファイルを保存したあとは、静的プロファイルと同じく `osvfs.toml` から
`aws-profile = "prod"` のように参照してください。

ロールクレデンシャルを更新したいときはいつでも同じコマンドを再実行できます。
実機での動作確認は次の手順で行えます:
