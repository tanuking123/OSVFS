...
が当てはまらないシナリオでは `--allow-unversioned` を指定して安全チェック
をバイパスできます。
...
| `--sync-interval-seconds` | 外部オブジェクトストア変更を検出するポーリング間隔。`0` で無効化。`polling` モードで使用 | `30` |
| `--change-source` | 外部オブジェクトストア変更の検出方式: `polling` (`--sync-interval-seconds` 周期でバケットを再列挙) または `events` (EventBridge S3 通知が流れる SQS キューを long-poll)。詳細は [変更検出モード](#変更検出モード) を参照 | `polling` |
| `--sync-mode` | ポーリング時の再列挙戦略: `on-demand` (ProjFS 経由でユーザーが訪問したディレクトリだけを再列挙。コストはバケットサイズではなく訪問ディレクトリ数に比例) または `full` (毎 tick バケット全体を再列挙。Phase&nbsp;1 当初の挙動)。詳細は [オンデマンド同期](#オンデマンド同期) を参照。`--change-source` が `polling` のときのみ有効 | `on-demand` |
| `--event-queue` | EventBridge S3 通知が流れる SQS キュー URL またはキュー名。`--change-source` が `events` の場合は **必須** | — |
...
| `--retry-max-attempts` | 一時的な失敗に対して AWS SDK が行う総試行回数 (初回 + リトライ)。`1` でリトライ無効。`4xx` エラーはこの値に関係なく即時失敗。詳細は [リトライポリシー](#リトライポリシー) | `3` |
| `--log-format` | コンソールログの出力形式: `text` (1 行ずつの人間可読形式) または `json` (1 行 1 JSON オブジェクト、UTC タイムスタンプ。Datadog / Loki などのログ集約基盤向け) | `text` |
| `--allow-unversioned` | **DANGER:** バケットバージョニングの安全チェックをスキップして、バージョニング無効のバケットに対して起動する。ローカルの編集・削除は復旧不可能になる。CI / 使い捨てバケット専用 — [バージョニングが必要な理由](#バージョニングが必要な理由) も参照 | off |
